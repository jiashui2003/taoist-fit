...
import { Scroll, Zap, Moon } from 'lucide-react';
...
export const BagView: React.FC<BagViewProps> = ({ metrics }) => {
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-24 pt-12 px-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="px-4 py-1 rounded-full bg-[#E6E2D0] text-sm text-[#4A4A4A]">关闭</button>
      </div>

      {/* Efficiency Report Card */}
      <div className="bg-[#FDFCF8] rounded-2xl p-6 shadow-sm border border-[#E6E2D0] mb-8">
        <div className="flex justify-between items-baseline mb-2">
           <span className="text-gray-500 text-sm">今日运动消耗</span>
           <span className="text-2xl font-mono font-bold">{metrics.calories} kcal</span>
        </div>
        <div className="flex justify-end text-[#BFA15F] text-sm font-medium mb-4">
           已转化灵气: {Math.floor(metrics.calories * 1.04)}
        </div>

        <div className="space-y-3 border-t border-dashed border-gray-200 pt-4">
           <div className="flex justify-between text-sm">
             <span>转化明细</span>
             <span className="font-mono text-gray-400">15:35 &nbsp; 8 kcal &nbsp; ×1.0 &rarr; 8 灵气</span>
           </div>
        </div>
